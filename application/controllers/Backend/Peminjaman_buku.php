...
class Peminjaman_buku extends Backend_Controller {
	...
	public function Aksibtnpeminjaman()
	{
		...
		if ($btnproses == 2) {
			$Sendmsg = $this->db->update('peminjaman', [
				'status' => $btnproses,
				'tgl_kembali' => date("Y-m-d"),
				'keterangan' => 'Dikembalikan Selesai..'
			]);
			if ($Sendmsg == TRUE) {
				$pesan['success'] = '<div class="alert alert-success bg-success text-white"> Berhasil Dikembalikan !! </div>';
			}else{
				$pesan['error'] = '<div class="alert alert-danger bg-danger text-white"> Gagal Dikembalikan !!';
			}
		} else {
